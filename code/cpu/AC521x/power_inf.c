...
static int wkup_reason_check(char *reason, int max_len)
{
    ...
    if (cnt >= 4) {
        ...
        tmp = rtc_wkup_reason();
        u8 short_key_on = 0;
        if (__this->charger_gpio() == IO_PORT_PR_02) {
            if (!(tmp & WKUP_IO_PR1)) { //not gsen wkup
                if (tmp & WKUP_IO_PR2) {
                    short_key_on = 1;
                }
            }
        } else {
            if (!(tmp & WKUP_IO_PR2)) { //not gsen wkup
                if (tmp & WKUP_IO_PR1) {
                    short_key_on = 2;
                }
            }
