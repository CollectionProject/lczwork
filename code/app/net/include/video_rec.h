...
struct video_rec_hdl {
    ...
    struct server *video_display_3;
    struct server *video_engine;


    struct vfscan *fscan[4];
    u16 file_number[4];
    u16 old_file_number[4];
    ...
    void *file[4];
    FILE *new_file[4];
    char fname[4][MAX_FILE_NAME_LEN];
...
struct net_video_hdl {
    enum VIDEO_REC_STA net_state;
    enum VIDEO_REC_STA net_state1;
...
extern u16 VIR_ENC_WIDTH;
extern u16 VIR_ENC_HEIGH;
extern int get_net_video_fps() ;
extern int get_audio_rate();
